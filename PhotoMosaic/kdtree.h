...
class KDTree
{
	...
	void buildTree(vector < Point<Dim> > & newPoints, int curDim, int left_index, int right_index);
	void find(vector< Point<Dim> >& newPoints, int left_index, int right_index, int median, int dim);
	int partition(vector< Point<Dim> >& newPoints, int left_index, int right_index, int pivotIndex, int dim);
	void swap(Point<Dim> & first, Point<Dim> & second);
	int pointDistance(const Point<Dim> & target, const Point<Dim> & currentBest) const;
        /**
	...
	Point<Dim> findNearestNeighbor(const Point<Dim> & query, int left, int right, int dim) const;
	bool otherTreeDistance(const Point<Dim> & query, const Point<Dim> & parent, int bestDistance, int dim) const;
